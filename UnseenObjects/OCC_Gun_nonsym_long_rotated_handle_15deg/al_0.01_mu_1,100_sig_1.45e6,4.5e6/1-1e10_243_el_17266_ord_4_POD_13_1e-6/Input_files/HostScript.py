...
from Functions.Saving.DictionaryList import DictionaryList
import shutil
import re
...
class bulk_sim_runner():
    ...
    def __init__(self, superclass):
        ...
        match superclass:
            ...
            case 'Weapons':
                allowed_classes = ['KnuckleDuster', 'Knife', 'Gun']
            case 'Clothing':
                allowed_classes = ['BeltBuckle', 'ShoeShank']
            case 'Tools':
                allowed_classes = ['Hammers', 'Scissors', 'Screwdriver']
    ...
    def write_latex(self):
        ...
        if os.path.exists(f'Object_Descriptions_{self.superclass}.tex'):
            os.remove(f'Object_Descriptions_{self.superclass}.tex')
        ...
        # # Closing file
        # with open(f"Doc_{self.superclass}.tex", 'a') as doc:
        #     lines = ['\n',
        #             '\n',
        #             r'\end{document}']
    ...
    def compute_object_info(self, sweep, mur, sig, inorout, mesh):
        print(sweep)
        if sweep == 'CSG_Knife_Knife_Cleaver_carbonsteel_copper_rivets.py':
            print('')
        ...
        inorout = []
        material_count = 0    
        with open(f'GeoFiles/{sweep[:-2]}geo', 'r') as geo:
            for line in geo:
                if line[:3] == 'tlo':
                    
                    hash_ind = line.index('#')
                    material_name = line[hash_ind:].strip().split(' ')[0][1:]
                    if material_name == 'air':
                        mur += [1]
                        sig += [0]
                        inorout += [0]
                    else:
                        mur_ind = line[hash_ind:].index('-')
                        sig_ind = line[hash_ind:].rindex('-')
                        mur += [float(line[(hash_ind+mur_ind):(hash_ind+sig_ind)][5:])]
                        sig += [float(line[(hash_ind+sig_ind):].strip()[5:])]
                        inorout += [1]
    ...
    def write_object_description(self, sweep, mat_list, mur_dict, sig_dict, prism_elements, tet_elements, volume, skin_depth, tau, superclass='Jewellery'):
        ...
        unique_mat_list = list(dict.fromkeys(mat_list))
        
        sweepname = f'Results/{sweep[:-3]}'
        sweepname += f'/al_0.001_mu_{DictionaryList(mur_dict, False)}_sig_{",".join(str(FtoS(sig_dict[x])) for x in unique_mat_list)}'
        sweepname += f'/1e1-1e10_243_el_{tet_elements+prism_elements}_ord_4_POD_13_1e-6/Graphs/'  
        
        classname = sweep.split('_')[1]
        formattedpath = fr'Class_{self.superclass}/Class_{classname}/OBJ_{"_".join(sweep[:-3].split("_")[2:-1])}'
        formattedpath += fr'/al_0.001_mu_1_sig_{",".join(str(FtoS(sig_dict[x])) for x in unique_mat_list)}'
        ...
        with open(f"texFiles/{sweep[:-3]}.tex", "w") as f:
            lines = [r'\begin{table}[!h]',
                        ...
                        fr'Class            & {self.superclass}, {classname}\\', '\n', 
                        fr'Formatted Path   & \url{{{formattedpath}}} \\', r'\hline', '\n'
                        fr'Volume [m$^3$]   & {volume} \\', '\n',
                        fr'Materials        & {",".join(str(x) for x in unique_mat_list)}\\'.replace('_', r'\_'), '\n',
                        fr'$\mu_r$          & {",".join(str(mur_dict[x]) for x in unique_mat_list)}\\', '\n',
                        fr'$\sigma_*$ [S/m] & {",".join(str(sig_dict[x]) for x in unique_mat_list)}\\', '\n',
                        fr'Skin Depth [m]   & {",".join(str(round(x,8)) for x in skin_depth)}\\', '\n',
                        fr'$\tau$           & {",".join(str(round(x,8)) for x in tau)} \\', '\n',
        ...
        with open(f"Object_Descriptions_{self.superclass}.tex", 'a') as doc:
    ...
    def _generate_preamble(self):
        with open(f"Doc_{self.superclass}.tex", 'w') as doc:
...
def autoformat_results(superclass='Jewellery'):
    ...
    match superclass:
        ...
        case 'Clothing':
            allowed_classes = ['BeltBuckle', 'ShoeShank']
        case 'Tools':
            allowed_classes = ['Hammer', 'Scissors', 'Screwdrivers']
    ...
    for files in sorted(os.listdir('Results')):
        ...
        classname = files.split('_')[1]
        if classname in allowed_classes:
            foldername = f'Class_{superclass}/Class_{classname}/OBJ_{objectname}'
            ...
            for children in os.listdir(foldername):
                for folder in os.listdir(foldername + '/' + children):
                    ...
                    if ((folderparts[0] == '1-1e10') or (folderparts[0] == '1e1-1e10'))  and (folderparts[1] == '243') and (folderparts[5] == '4') and (folderparts[7] == '13') and (folderparts[8] == '1e-6'):
                        pass
                    else:
...
if __name__ == '__main__':
    ...
    print('\n'.join(runner.filelist))
    runner.run_sims()
    # runner.write_latex()
    
    # autoformat_results(superclass='Weapons')
