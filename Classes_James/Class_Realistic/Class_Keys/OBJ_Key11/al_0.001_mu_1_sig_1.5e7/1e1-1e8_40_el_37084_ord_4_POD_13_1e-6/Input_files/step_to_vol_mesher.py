...
def step_mesher(filename, output_filename='outputstep.vol', add_boundary_layer=False,
    ...
    if add_boundary_layer is True:
        ...
        mesh.BoundaryLayer(boundary=".*", thickness=[1e-2, 5e-2, 5e-1], material=mat,
    ...
    with open(geo_filename, 'w') as file:
        ...
        if boundary_layer_mat_name != 'default':
            file.write('\n')
            file.write(
                'tlo ' + boundary_layer_mat_name + '_layer -col=[1,0,0];#mat1 -mur=' + str(mur_layer) + ' -sig=' + str(sigma_layer))
...
if __name__ == '__main__':
    ...
    output_filename = 'remeshed_matt_key_11.vol'
