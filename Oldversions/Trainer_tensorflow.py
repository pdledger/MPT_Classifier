...
Models_to_run = ['TensorflowNN']#'LogisticRegression']#,'SVM', 'DecisionTree', 'RandomForest', 'GradientBoost', 'MLP']
...
#                    exec('model = MLPClassifier(hidden_layer_sizes='+Layers+',random_state=1, max_iter=1000).fit(X_train_norm, Y_train)')
        ...
        if Load_External_Data is True:
            ...
            if (Model in Pobabalistic_Classifiers) or ('MLP' in Model) or (Model=='TensorflowNN'):
