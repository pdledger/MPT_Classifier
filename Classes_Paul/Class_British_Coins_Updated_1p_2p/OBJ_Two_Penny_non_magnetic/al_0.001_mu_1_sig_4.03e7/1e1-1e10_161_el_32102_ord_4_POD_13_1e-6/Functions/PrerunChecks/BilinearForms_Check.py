...
def BilinearForms_Check(mesh, order, mu_inv, sigma, inout, bilinearform_tol, max_iter, curve_order, starting_order, sweepname):
   ...
   # alpha = 0.01
   # nu_no_omega = alpha**2 * 4*np.pi*1e-7
   # omega = 1e8
   # epsi = 1e-10
    
    # Checking theta1 bilinear form: NOT INTENDED TO BE STANDARD
    ...
    #a += SymbolicBFI((mu_inv) * InnerProduct(curl(u), curl(v)), bonus_intorder=bonus_intord)
    #a += SymbolicBFI((1j) * inout * nu_no_omega * omega * sigma * InnerProduct(u, v), bonus_intorder=bonus_intord)
    #a += SymbolicBFI((1j) * (1 - inout) * epsi * InnerProduct(u, v), bonus_intorder=bonus_intord)
    #a.Assemble()
    ...
    #while (rel_diff > bilinearform_tol) and (counter < max_iter):
    #    print(f'a: Iteration {counter}: bonus_intord = {bonus_intord}')
   ...
   # plt.figure()
   # plt.title(f'Curve order: {curve_order}')
   # plt.plot(ord_array, rel_diff_array, '*-', label='Relative Difference a')
   # plt.axhline(bilinearform_tol, color='r', label='Tolerance')
   # plt.xlabel('Integration Order')
   # plt.ylabel('Relative Difference K')
   # plt.yscale('log')
   # plt.legend()
   # a_order = bonus_intord
   # print(a_order)
