...
def Creator(Creator_Settings):
    # Extract all the settings from the Creator_Settings dictionary.
    Name = Creator_Settings["Name"]

    Frequencies = Creator_Settings["Frequencies"]

    Classes = Creator_Settings["Classes"]

    extend_results = Creator_Settings["extend_results"]

    Num_Results = Creator_Settings["Num_Results"]

    class_split = Creator_Settings["class_split"]

    class_names = Creator_Settings["class_names"]

    Num_Results_class = Creator_Settings["Num_Results_class"]

    class_num_results = Creator_Settings["class_num_results"]

    Label_Data = Creator_Settings["Label_Data"]

    Name_Objects = Creator_Settings["Name_Objects"]

    Object_Names_dictionary = Creator_Settings["Object_Names_dictionary"]

    Name_Order = Creator_Settings["Name_Order"]

    Scale_type = Creator_Settings["Scale_type"]

    Scale_File = Creator_Settings["Scale_File"]
...
##    #(float) percentage of original if Scale_type = 'Global' (this is a standard deviation)
    ...
    with open('DataSets/'+Name+'/Creator_Settings.csv', 'w') as fset:
        fset.write(str(np.asarray(Creator_Settings)))
