...
def VolMatUpdater(Geometry,OldMesh,smur,ssig):
    #Remove the .geo part of the file extention
    objname=Geometry[:-4]
    ...
    if OldMesh == False:
        ...
        f.close()
        f=open(objname+".vol","w")
        for line in newfile:
            f.write(line)
        f.close()
    
    
    #Recompute the murlist and siglist
    
    #Create the new list
    murlist2 = []
    siglist2 = []
    
    #Check for multiple regoins
    smur.split(';')
    ssig.split(';')
    if type(smur) == str:
        smur = smur.replace('[','').replace(']','')
        ssig = ssig.replace('[','').replace(']','')
        fmur = [float(smur)]
        fsig = [float(ssig)]
    else:
        fmur = [float(i.replace('[','').replace(']','')) for i in smur]
        fsig = [float(i.replace('[','').replace(']','')) for i in ssig]
    
    j = 0
    for i,mu in enumerate(murlist):
        if matlist[i]!='air':
            murlist2.append(float(fmur[j]))
            siglist2.append(float(fsig[j]))
            j += 1
        else:
            murlist2.append(1)
            siglist2.append(0)
    ...
    mur=dict(zip(matlist,murlist2))
    sig=dict(zip(matlist,siglist2))
